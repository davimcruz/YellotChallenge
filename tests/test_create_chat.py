...
@pytest.mark.asyncio
async def test_create_message(test_client, test_db):
    # Criar usuário e fazer login (reaproveitando código do test_create_chat_room)
    user1 = await test_client.post("/api/v1/users/", json={
        "username": "testuser1",
        "email": "test1@example.com",
        "password": "password123"
    ...
    login = await test_client.post("/api/v1/users/login", json={
        "email": "test1@example.com",
        "password": "password123"
    })
    ...
    assert data["content"] == "Olá, como vai?"
    assert data["sender_id"] == user1_data["id"]
    assert data["sender_username"] == "testuser1"
